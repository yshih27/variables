...
SELECT COUNT(*) AS txns_30d, SUM(amount) AS volume_30d,
  COUNT(*) FILTER (WHERE block_time > now() - interval '7' day) AS txns_7d,
  SUM(amount) FILTER (WHERE block_time > now() - interval '7' day) AS volume_7d,
  COUNT(*) FILTER (WHERE block_time > now() - interval '24' hour) AS txns_24h,
