...
--
-- ⚠️ WINDOWED to 35d — keep it that way. Unbounded, these four scans burned
...
-- ts), so re-deriving only the trailing 35d loses nothing. Narrowed 90d → 35d
-- because these full-table scans are the dominant Dune credit driver and Dune
-- bills executions by COMPUTE — same principle as the Courtyard 30d fix (#65).
-- 35d still covers every window the readers use (24h / 7d / 30d) with a few
-- days of slack for a late or corrected day.
