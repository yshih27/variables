      ...
      <div className="flex flex-col gap-0.5 p-2">
        {items.map((ip) => (
          <a
            key={ip.key}
            href={`/ip/${ip.key}`}
            className="grid grid-cols-[24px_36px_1fr_auto_auto] items-center gap-3.5 rounded-[10px] px-3.5 py-3 transition-colors hover:bg-bg-2"
          >
            ...
            <span className="flex flex-col">
              <span className="text-[14px] font-semibold">{ip.name}</span>
