...
/**
 * Activity — multi-metric overlay chart for the IP page.
 *
 ...
 *
 * DATA: `volume` 24H is real (hourly buckets). Other metrics / longer windows
 * are placeholder series until the backend `metric_snapshots` spine fills —
 * swap `series` for `readMetricSeries("ip", key, metric)` then. Chip headline
 * values are always the real current figures.
 */

export type Timeframe = "24H" | "7D" | "30D" | "ALL";
export const TIMEFRAMES: Timeframe[] = ["24H", "7D", "30D", "ALL"];

export type MetricWindow = { points: number[]; real: boolean };
export type ActivityMetric = {
  key: string;
  label: string;
  color: string; // CSS color
  value: string; // real current headline value (shown on the chip)
  /** Points per timeframe (oldest → newest) + whether they're real or sample. */
  series: Record<Timeframe, MetricWindow>;
};

const H = 320;
const PAD = { top: 16, right: 64, bottom: 30, left: 6 };
...
function xLabels(tf: Timeframe, n: number): string[] {
...
export function IPActivityChart({ metrics }: { metrics: ActivityMetric[] }) {
  ...
  const labels = xLabels(tf, hoverN);
    ...
    <section className="mb-12 font-sans">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {metrics.map((m) => {
          ...
          const sample = !(m.series[tf]?.real ?? true);
          return (
            <button
              key={m.key}
              type="button"
            ...
            >
              <span
                ...
                style={{ background: m.color, boxShadow: on ? `0 0 8px ${m.color}` : "none" }}
              />
        ...
        {hover != null && lines.length > 0 && lines[0].pts[hover] && (
          ...
          >
            <div className="mb-1.5 font-mono text-[10.5px] uppercase tracking-[0.08em] text-ink-3">
              {labels.length ? hoverLabel(tf, hover, hoverN) : ""}
            </div>
            {lines.map((m) => (
...
function hoverLabel(tf: Timeframe, idx: number, n: number): string {
  ...
  if (tf === "7D" || tf === "30D") {
    const days = Math.round((ago / (n - 1)) * (tf === "7D" ? 7 : 30));
    return days === 0 ? "now" : `${days}d ago`;
  }
  return ago === 0 ? "now" : `${ago} pts ago`;
}

/** Real per-point value when the window is real; headline value when sampled. */
function pointValue(m: ActivityMetric & { pts: Array<[number, number]> }, tf: Timeframe, idx: number): string {
  const win = m.series[tf];
  if (win?.real) {
    const raw = win.points?.[idx];
    if (raw != null && Number.isFinite(raw)) {
      if (m.key === "volume" || m.key === "marketCap" || m.key === "avgTrade") {
